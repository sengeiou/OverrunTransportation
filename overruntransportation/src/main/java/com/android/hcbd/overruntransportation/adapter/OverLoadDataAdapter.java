...
public class OverLoadDataAdapter extends BaseAdapter{
    private Context context;
    private List<OverLoadDataInfo.DataInfo> list;
    private int position;
    public OverLoadDataAdapter(Context context, List<OverLoadDataInfo.DataInfo> list){
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        if(view == null){
            view = LayoutInflater.from(context).inflate(R.layout.item_overload_data_layout,null);
            holder = new ViewHolder(view);
            view.setTag(holder);
        }else{
            holder = (ViewHolder) view.getTag();
        }
        holder.tv_name.setText((i+1)+"、"+list.get(i).getCompany().getShortName());
        holder.tv_code.setText("检测车辆（辆）："+list.get(i).getD1());
        holder.tv_operNames.setText("超限车辆（辆）："+list.get(i).getD2());
            ...
            public void onClick(View view) {
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int position) {
                        ...
                        OkGo.<String>post(MyApplication.getInstance().getBsaeUrl() + HttpUrlUtils.delete_overload_data_url )
                                .params("sessionOper.code", MyApplication.getInstance().getLoginInfo().getUserInfo().getCode())
                                .params("sessionOper.orgCode",MyApplication.getInstance().getLoginInfo().getUserInfo().getOrgCode())
                                .params("token",MyApplication.getInstance().getLoginInfo().getToken())
                                .params("tpsData.id",list.get(i).getId())
                                    ...
                                    public void onSuccess(Response<String> response) {
                                        ...
                                        try {
                                            ...
                                            if(!TextUtils.isEmpty(jsonObject.getString("data"))){
                                                ...
                                                MessageEvent messageEvent = new MessageEvent(MessageEvent.EVENT_OVERLOAD_DATA_LIST_DEL);
