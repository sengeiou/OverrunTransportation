...
public class DefaultPicNameAddActivity extends BaseActivity implements View.OnClickListener {
    ...
    CheckBox cb_close;
    private String type;
    private DefaultProblemInfo.DataInfo dataInfo;
    private DialogUtils dialogUtils = new DialogUtils();
    private List<DefaultProblemInfo.DataInfo> dataInfoList;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_default_pic_name_add);
        ButterKnife.bind(this);

        type = getIntent().getStringExtra("type");
        dataInfo = (DefaultProblemInfo.DataInfo) getIntent().getSerializableExtra("data");
        dataInfoList = (List<DefaultProblemInfo.DataInfo>) getIntent().getSerializableExtra("list");
    ...
    private void initData() {
        ...
        if(type.equals("add")){
            tv_title.setText("默认图片名称录入");
        }else{
            tv_title.setText("默认图片名称修改");
        ...
        switch (view.getId()){
            ...
            case R.id.iv_save:
                if(TextUtils.isEmpty(et_code.getEditableText().toString())){
                    ToastUtils.showShortToast(this,"请输入默认图片名称编号");
                    return;
                }
                if(TextUtils.isEmpty(et_name.getEditableText().toString())){
                    ToastUtils.showShortToast(this,"请输入默认图片名称");
                    return;
                }
                if(type.equals("add")){
                    boolean isAdd = false;
    ...
    private void saveData() {
        ...
        OkGo.<String>post(MyApplication.getInstance().getBsaeUrl()+HttpUrlUtils.add_edit_default_problem_url)
                ...
                .params("typeNote", "默认图片列表")
                .execute(new StringCallback() {
                    @Override
                    public void onStart(Request<String, ? extends Request> request) {
                        super.onStart(request);
                        dialogUtils.showLoading(DefaultPicNameAddActivity.this);
                    }
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        try {
                            ...
                            if(!TextUtils.isEmpty(jsonObject.getString("data"))){
                                ...
                                if(type.equals("add"))
                                    ToastUtils.showShortToast(DefaultPicNameAddActivity.this,"添加成功！");
                                else
                                    ToastUtils.showShortToast(DefaultPicNameAddActivity.this,"修改成功！");
                                MessageEvent messageEvent = new MessageEvent();
                                if(type.equals("edit")){
                                    messageEvent.setEventId(MessageEvent.EVENT_DEFAULT_PROBLEM_LIST_EDIT);
                                    messageEvent.setObj(dataInfo);
                                }else{
                                    messageEvent.setEventId(MessageEvent.EVENT_DEFAULT_PROBLEM_LIST_ADD);
                                }
